...
public class FruitBasket {
	...
	public static void main(String[] args) {
		...
		case 1:
			...
			if(count< arr.length) 
				arr[count++]=new Mango(sc.next(),sc.nextDouble(),sc.next(),true);
		...
		case 2:
			...
			if(count< arr.length) {
					arr[count++]=new Orange(sc.next(),sc.nextDouble(),sc.next(),true);
			}
			break;
		case 3:
			System.out.println("Color Weight Name");
			if(count< arr.length) {
				arr[count++]=new Apple(sc.next(),sc.nextDouble(),sc.next(),true);
			}
			break;
		case 4:
			for(Fruit f :arr) {
		...
		case 6:
        	...
        	arr[sc.nextInt()-1].isFresh(false);
			break;
